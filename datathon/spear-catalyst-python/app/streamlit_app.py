...
sys.path.insert(0, './lib')                                    # Dependencies installed by Catalyst predeploy
...
with tab_over:
    ...
    with st.container(border=True):
        fig = px.line(monthly, x="ym", y="cases", title="Cases per month", markers=True)
        fig.update_traces(line=dict(width=2.2), marker=dict(size=6))
        st.plotly_chart(spear_fig(fig, height=340), use_container_width=True)

    with st.container(border=True):
        fig = px.bar(f["CrimeType"].value_counts().head(10)[::-1],
                     orientation="h", title="Top 10 crime types")
        fig.update_traces(marker_color=SERIES[0])
        fig.update_layout(showlegend=False)
        st.plotly_chart(spear_fig(fig, height=380), use_container_width=True)
...
with tab_map:
    ...
    with st.container(border=True):
        ...
        fig.update_traces(marker_color=SERIES[0])
        st.plotly_chart(spear_fig(fig, height=360), use_container_width=True)
    ...
    if summ.empty:
        ...
        with st.container(border=True):
            fig = px.scatter_mapbox(
                ...
                height=600, title="Detected crime hotspots (each color = one cluster)")
            st.plotly_chart(spear_fig(fig, height=600), use_container_width=True)
...
with tab_time:
    ...
    with st.container(border=True):
        ...
        st.plotly_chart(spear_fig(fig, height=450), use_container_width=True)
...
with tab_alert:
    ...
    if alerts.empty:
        ...
        with st.container(border=True):
            ...
            fig.update_traces(marker_color=SERIES[0])
            fig.add_vline(x=top.ym, line_color=T["critical"], line_dash="dash")
            st.plotly_chart(spear_fig(fig, height=380), use_container_width=True)
