...
@st.cache_data(show_spinner=False)
def _metrics():
    """Sealed Module 2 exam results — quoted, never recomputed (SWD rule 6)."""
    import json
    from pathlib import Path
    try:
        p = Path(__file__).resolve().parents[1] / "reports" / "module2_metrics.json"
...
def render_network_tab(db_path: str):
    ...
    # SWD rule 6 — the headline IS the finding, with the validated number attached
    _m = _metrics()
    _f1 = _m.get("entity_resolution", {}).get("f1")
    _rings = _m.get("rings", {})
    _rec, _pl = _rings.get("recovered"), _rings.get("planted")
    _bits = [f"{n_accused:,} accused records collapse to {len(prof):,} people"]
    if _rec is not None and _pl is not None:
        _bits.append(f"{_rec}/{_pl} planted rings surface")
    if _f1 is not None:
        _bits.append(f"ER F1 {_f1}")
    st.subheader("🕸️ " + " — ".join(_bits[:2]) +
                 (f" ({_bits[2]} vs sealed ground truth)" if len(_bits) > 2 else ""))
    ...
    # --- interactive graph --------------------------------------------------
    _kp = _m.get("kingpin_top10_ring_hits")
    st.markdown(
        f"**Louvain recovers {int((cp.Size >= 4).sum())} organized rings"
        + (f", and {_kp}/10 top-betweenness entities are confirmed ring members"
           if _kp is not None else "")
        + "** — each colour is one ring, node size is degree centrality.")
